...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body style="margin: 0; padding: 0;">
        ...
        <tr>Место проведения мероприятия: ${location}</tr>
        <tr>Сумма заказа: ${totalPrice}</tr>
        ${priceInfo.map((el) => {
          return `<br>${el.title}: ${el.price}р`;
        })}
        <tr style="margin-top: 30px; display: block">Имя: ${name}</tr>
        <tr>Дата мероприятия: ${date}</tr>
        <tr>Телефон: ${phone}</tr>
        </table>
    </body>
</html>`;
